...
const Main: FC<Props> = (props) => {
  ...
  const renderFilmPromo = (): React.ReactNode => {
    if (!filmPromo) {
      return null;
    }
    const { id, title, genre, poster_image } = filmPromo;
    return (
  ...
  const renderTitlePromo = () => {
    if (!filmPromo) {
      return null;
    }
    const { title, background_image } = filmPromo;
    return (
